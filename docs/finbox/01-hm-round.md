...
| **Data Vault 2.0** | "Conceptually — hubs, links, satellites; insert-only, source-agnostic, audit-friendly, and expensive to query without a mart on top. I've modelled financial facts as star-shaped ledgers; I'd reach for Vault where you have many source systems and a hard audit requirement, which for a lender integrating hundreds of partners might be exactly the case." The [modelling doc](../data-engineering/data-modelling.md) has the mechanics (hash keys, `hash_diff`, PIT/bridge, raw vs business vault), the [worked lending model](../data-engineering/data-modelling.md#6-a-lending-domain-model-worked) and [the HM Q&A](../data-engineering/data-modelling.md#7-questions-an-hm-will-ask). |
...
5. **Do the reading you'll claim** ([§J](#j-the-gap-questions)): ClickHouse MergeTree /
   ...
   semantics; Delta vs Iceberg. Data modelling is done — read
   [data-modelling.md](../data-engineering/data-modelling.md) and be able to draw
   [§6](../data-engineering/data-modelling.md#6-a-lending-domain-model-worked) from memory.
   Say only what you've actually read.
